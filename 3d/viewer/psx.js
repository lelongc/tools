...
// ==========================================
// MODEL LOADER (GLTF & FBX with BoxGeometry fallback)
// ==========================================
...
function loadModelOrFallback(modelPath, fallbackFn, scale = 1.0) {
    return new Promise((resolve) => {
        ...
        if (isFbx) {
            ...
            const resourcePath = lastSlash !== -1 ? modelPath.substring(0, lastSlash + 1) : '';
            fbxLoader.setResourcePath(resourcePath);
            fbxLoader.load(
                modelPath,
                (fbx) => {
                    applyPsxFilters(fbx);
                    modelCache[modelPath] = fbx;
                ...
                undefined,
                (err) => {
                    console.warn(`Failed to load FBX model: ${modelPath}, using fallback.`, err);
                    resolve(fallbackFn());
                }
            );
        } else {
            gltfLoader.load(
                ...
                undefined,
                (err) => {
                    console.warn(`Failed to load GLTF/GLB model: ${modelPath}, using fallback.`, err);
                    resolve(fallbackFn());
                }
            );
        }
