...
# Cell 2: Ultra-dense speech splitting (1.5s - 2.5s per image)
...
if not found_audio:
    ...
    # ĐỒNG BỘ DÀY ĐẶC (Cứ 1.8s - 2.5s ngắt 1 cảnh để video cực kỳ sinh động)
    ...
    for segment in result['segments']:
        for word in segment['words']:
            ...
            # Ngắt phân cảnh khi đạt từ 1.8s trở lên
    ...
    print(f"🎉 Đã tạo thành công {len(scenes)} phân cảnh sinh động (Trung bình 2s/ảnh)!")
"""

# Cell 5: Perfect XML Generator matching 1.scribe 100%
cell_5_source = """def generate_scribe_project():
    ...
    # Gom 2-3 hình vào 1 khung cảnh đểlia camera chuyên nghiệp như 1.scribe
    scenes = []
    current_scene = []
    ...
    with open(xml_path, "w", encoding="utf-8") as f:
        ...
        for scene_idx, scene in enumerate(scenes):
            ...
            for i, item in enumerate(scene):
                ...
                # Vị trí vừa vặn trong khung hình
                if n == 1:
                ...
                # Camera Zoom & Focus chuẩn 1.scribe
                if i == n - 1 and n > 1:
                    cam_scale = 0.82 # Zoom out vừa đủ bao quát
                    cam_x = 448.5 - scene_x * cam_scale
                    cam_y = 252.5 - scene_y * cam_scale
                else:
                    cam_scale = 1.05 # Focus cận cảnh
                ...
                movin_arc = random.choice(['0', '1'])
                
                # KHẮC PHỤC TRIỆT ĐỂ LỖI ĐƠ: Giới hạn targetTime từ 600ms đến tối đa 2200ms
                # Không bao giờ để tay vẽ kéo dài > 10s gây đơ phần mềm
                target_time_ms = max(600, min(2200, int(duration * 600)))
                pause_time_ms = 500
                trans_time_ms = 500
                ...
                # Thuộc tính chuẩn 100% giống file 1.scribe mẫu
                element_xml = (
    ...
    print(f"\\n🎉 ĐÃ HOÀN TẤT! File dự án: {output_filename} (Bản AI Director 5.0 Siêu Chuẩn!)")
