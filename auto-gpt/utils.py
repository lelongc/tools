...
def extract_verification_code(content: str):
    ...
    # Bước 1: Loại bỏ HTML tags để tránh match vào attributes, hex colors, etc.
    ...
    print(f"  [DEBUG extract] clean_text (200 chars): {clean_text[:200]}")
    
    # Bước 2: Patterns có ngữ cảnh rõ ràng (ưu tiên cao nhất)
    context_patterns = [
        r'代码为\s*(\d{6})',
        r'code\s+is\s*:?\s*(\d{6})',
        r'verification\s+code\s*:?\s*(\d{6})',
        r'verify\s+code\s*:?\s*(\d{6})',
        r'code\s*:\s*(\d{6})',
        r'码\s*[:：]\s*(\d{6})',
    ]
    ...
    for pattern in context_patterns:
        ...
        if match:
            code = match.group(1)
            print(f"  ✅ 提取到验证码 (context match): {code}")
            return code
    
    # Bước 3: Tìm tất cả 6-digit numbers trong clean text, lọc bỏ false positives
    all_matches = list(re.finditer(r'(?<!\d)(\d{6})(?!\d)', clean_text))
    print(f"  [DEBUG extract] found {len(all_matches)} 6-digit candidates in clean text")
    
    for match in all_matches:
        code = match.group(1)
        start_pos = match.start(1)
        
        # Lấy context xung quanh (30 chars trước, 30 chars sau)
        prefix = clean_text[max(0, start_pos - 30):start_pos].lower()
        suffix = clean_text[start_pos + 6:start_pos + 36].lower()
        
        print(f"  [DEBUG extract] candidate={code}, prefix='{prefix}', suffix='{suffix}'")
        
        # Bỏ qua nếu thuộc boundary
        if 'boundary' in prefix or '_' in prefix[-5:]:
            print(f"  [DEBUG extract] skip {code}: boundary/underscore")
            continue
        
        # Bỏ qua nếu là một phần của URL
        if 'http' in prefix or '/' in prefix[-3:] or '.' in prefix[-3:]:
            print(f"  [DEBUG extract] skip {code}: URL-like")
            continue
        
        # Bỏ qua nếu nằm trong chuỗi hex dài (ví dụ color codes, hash values)
        surrounding = clean_text[max(0, start_pos - 5):start_pos + 11]
        if re.match(r'[0-9a-fA-F]{8,}', surrounding.replace(' ', '')):
            print(f"  [DEBUG extract] skip {code}: hex string")
            continue
        
        # Bỏ qua nếu theo sau bởi 'px', 'em', '%' (CSS values)
        if re.match(r'(px|em|%|pt|rem)', suffix.strip()):
            print(f"  [DEBUG extract] skip {code}: CSS value")
            continue
        
        print(f"  ✅ 提取到验证码 (fallback match): {code}")
        return code
    
    # Bước 4: Thử tìm trực tiếp trong content gốc (chưa clean) nếu clean không tìm thấy
    for pattern in context_patterns:
        match = re.search(pattern, content, re.IGNORECASE)
        if match:
            code = match.group(1)
            print(f"  ✅ 提取到验证码 (raw content match): {code}")
            return code
