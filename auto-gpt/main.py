...
def register_one_account(monitor_callback=None):
    ...
    try:
        ...
        # 5. 填写注册表单（邮箱和密码）
        signup_result = fill_signup_form(driver, email, password, jwt_token)
        if not signup_result or not signup_result.get("success"):
            print("❌ 填写注册表单失败")
            return email, password, False
        _report("fill_form")
        
        # 6 & 7. 等待并输入验证码 (仅当 OTP chưa được xác minh ở bước trên)
        if not signup_result.get("otp_verified"):
            # 等待验证邮件
            time.sleep(5)
            ...
            # Luồng mới: OTP đã được xác minh trong fill_signup_form
            _report("enter_code")
