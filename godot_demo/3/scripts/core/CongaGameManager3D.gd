...
## CongaGameManager3D.gd
## Điều phối vòng đấu 3D 1-4 người chơi, tính điểm và bảng xếp hạng Vua Quất Roi
...
func bank_mannequins(player_id: int, count: int) -> int:
	...
	CongaSoundManager3D.play_sfx("bank_score", 1.0)
	scores_updated.emit(scores, chain_lengths)
	return pts

func register_whip_hit(attacker_id: int) -> void:
	total_whips_landed[attacker_id - 1] += 1
	scores[attacker_id - 1] += 60
	CongaSoundManager3D.play_sfx("whip_hit", randf_range(0.9, 1.3))
	scores_updated.emit(scores, chain_lengths)
...
func _finish_match() -> void:
	...
	CongaSoundManager3D.play_sfx("cheer", 1.0)
