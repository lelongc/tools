...
var star_texture = preload("res://textures/env/star_gold.svg")
...
func setup(stars: int, final_score: int = 0) -> void:
	...
	for i in range(stars):
		...
		tween.tween_property(tr, "scale", Vector2.ONE, 0.25)
...
func _ready() -> void:
	...
	if PuffyGameManager.current_level >= PuffyGameManager.max_levels:
		next_btn.text = "🏆 PHÁ ĐẢO TOÀN BỘ 30 MÀN!"
