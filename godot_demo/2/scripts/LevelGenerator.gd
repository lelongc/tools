...
var gate_scene: PackedScene = preload("res://scenes/Gate.tscn")
...
func spawn_gates_and_traps() -> void:
	...
	for i in range(z_positions.size()):
		...
		if i == 0:
			left_gate.gate_type = 0 # FIRE_RATE_ADD
			left_gate.value = 5.0
			right_gate.gate_type = 3 # BULLET_COUNT_ADD
			right_gate.value = 1.0
		elif i == 1:
			left_gate.gate_type = 1 # FIRE_RATE_MULT
			left_gate.value = 1.5
			right_gate.gate_type = 2 # FIRE_RATE_SUB
			right_gate.value = -3.0
		elif i == 2:
			left_gate.gate_type = 3 # BULLET_COUNT_ADD
			left_gate.value = 2.0
			...
			left_gate.gate_type = 5 # POWER_ADD
			left_gate.value = 15.0
			right_gate.gate_type = 0 # FIRE_RATE_ADD
			right_gate.value = 8.0
		else:
			left_gate.gate_type = 4 # BULLET_COUNT_MULT
			left_gate.value = 2.0
			right_gate.gate_type = 1 # FIRE_RATE_MULT
		...
		if i < z_positions.size() - 1:
			...
			if obs.has_method("update_label"):
				obs.update_label()
...
func spawn_boss_arena() -> void:
	...
	trigger_area.body_entered.connect(func(body):
		if body.has_method("reach_boss_arena") and is_instance_valid(boss_instance):
