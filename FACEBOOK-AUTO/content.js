    ...
    try {
      ...
      for (const img of images) {
        ...
        if (btn) {
          ...
          if (rect.width > 2 * rect.height) return btn;
        }
      }
    } catch (err) {}
...
function f(e) {
  e && ["mouseover", "mousedown", "mouseup", "click"].forEach(t => {
...
async function insertPostText(postText, postTitleText) {
  ...
  function getEditors() {
    ...
    return Array.from(container.querySelectorAll(editorSelectors)).filter(e => {
      const rect = e.getBoundingClientRect();
      return rect.width > 20 && rect.height > 10 && null !== e.offsetParent;
    });
  }
  ...
  if (postTitleText) {
    ...
    await y(1.5); // Wait for dynamic Title field to lazy-load
    ...
    for (let attempt = 0; attempt < 10; attempt++) {
      ...
      if (titleCandidates.length > 0) {
        ...
        const bestTitle = titleCandidates.find(e => {
          const label = (e.getAttribute("aria-label") || "").toLowerCase();
          const placeholder = (e.getAttribute("placeholder") || "").toLowerCase();
          const tag = e.tagName.toLowerCase();
          return label.includes("title") || label.includes("tiêu đề") || placeholder.includes("title") || placeholder.includes("tiêu đề") || tag === "input" || tag === "textarea";
        });
        if (bestTitle) {
          titleElement = bestTitle;
        } else {
          titleCandidates.sort((a, b) => b.getBoundingClientRect().top - a.getBoundingClientRect().top);
          titleElement = titleCandidates[0];
        }
        break;
      }
      await y(0.5);
      ...
      try {
        ...
        const t = await function (e = 15, t = 1e3) {
          return new Promise(async n => {
            ...
            console.log("Post button still not found after retries."), chrome.storage.local.set({ operationStatus: "failed" }), n(!1);
          });
        }();
        ...
        try {
          ...
          let e = await p();
          "pending" == e && (s = !0, chrome.storage.local.set({ operationStatus: "pending" })), "restricted" == e && (s = !0, chrome.storage.local.set({ operationStatus: "restricted" })), s || (console.error("Post submission failed - submit button still present after timeout"), chrome.storage.local.set({ operationStatus: "failed" }));
        } catch (e) {
          console.error("Error during post verification:", e);
          let t = await p();
          "pending" == t ? (s = !0, chrome.storage.local.set({ operationStatus: "pending" })) : "success" == t ? chrome.storage.local.set({ operationStatus: "success" }) : "restricted" == t ? chrome.storage.local.set({ operationStatus: "restricted" }) : chrome.storage.local.set({ operationStatus: "failed" });
        }
        if (!r) {
          console.error("Post submission failed due to timeout.");
          let e = await p();
          "pending" == e ? (s = !0, chrome.storage.local.set({ operationStatus: "pending" })) : "success" == e ? chrome.storage.local.set({ operationStatus: "success" }) : "restricted" == e ? chrome.storage.local.set({ operationStatus: "restricted" }) : chrome.storage.local.set({ operationStatus: "failed" });
        }
      } catch (e) {
        console.error("An error occurred during post creation:", e), chrome.storage.local.set({ operationStatus: "failed" });
      }
    });
  }
  return !0;
